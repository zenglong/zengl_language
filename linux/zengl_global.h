...
typedef struct _ZENGL_RUN_VIRTUAL_MEM_LIST{
	...
	ZENGL_RUN_HASH_ARRAY hash_array; // 当使用字符串作为key来访问mem_array中的成员时，先根据hash_array将key转为整数索引值，再由索引值来访问具体的成员
...
typedef struct _ZENGL_COMPILE_TYPE
{
	...
	ZENGL_SYM_SELF_CLASS_TABLE SymSelfClassTable; // self节点类信息动态数组
