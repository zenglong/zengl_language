...
ZL_VOID zengl_AsmGenCodes(ZL_VOID * VM_ARG,ZL_INT nodenum)
{
	...
	while(state != ZL_ST_DOWN)
	{
		...
		case ZL_ST_INID: //fun函数定义的参数部分如 fun test(a,b) a和b都是id标识符，经过逗号分隔开后，a和b都会经过AsmGenCodes函数到达这里，所以需要将a和b的值先赋值给AX,在AsmGenCodes函数结束时再将AX压入栈，从而使得a,b的值能成为fun test函数的参数。
		...
		case ZL_ST_INASSIGN: //赋值运算符汇编输出
			...
			switch(nodes[chnum[1]].toktype) //如果第二个子节点为ID标示符变量，就输出MOV AX (变量内存地址) ， 索引0表示第一个子节点，1则是第二个子节点，将该节点变量内存空间的值赋给AX寄存器
			{
			case ZL_TK_ID:
			...
			default:
				if(ZENGL_AST_ISTOKEXPRESS(chnum[1]))
					compile->AsmGenCodes(VM_ARG,chnum[1]); //递归调用AsmGenCodes生成第二个子节点对应的表达式的汇编代码，该段汇编代码执行完后，一般会将结果保存在AX寄存器中，供之后的汇编指令使用。
				else
				{
...
assign: //加赋值，减赋值等运算符在生成加减等汇编代码后，就会跳到此处来生成赋值语句的汇编代码
			...
			case ZL_TK_DOT:
				...
				compile->AsmGenCodes(VM_ARG,chnum[0]); //生成数组元素或类成员的汇编代码。
			...
			if(nodes[nodenum].childs.count == 2) //判断节点是否有两个子节点
			{
				...
				default:
					if(ZENGL_AST_ISTOKEXPRESS(chnum[1]))
					{
						run->AddInst(VM_ARG, compile->gencode_struct.pc++, chnum[1],
								ZL_R_IT_PUSH , ZL_R_DT_NONE , 0,
								ZL_R_DT_REG , ZL_R_RT_AX); //对应汇编指令 "PUSH AX" 先将前面的AX寄存器里的值压入栈，下面的AsmGenCodes会将新值传入AX
		...
		case ZL_ST_ASM_CODE_INADDRESS: //引用运算符的处理
			...
			break; //case ZL_ST_ASM_CODE_INADDRESS: //引用运算符的处理
		case ZL_ST_ASM_CODE_IN_PP_MM: //++和--运算符，这两个运算符的作用也和C语言的一样，就是对变量或表达式结果加一或减一，当++在前面时，先加一再取值，当++在后面时，先取值后加一，--同理
			...
			break; //case ZL_ST_ASM_CODE_IN_PP_MM: //++和--运算符，这两个运算符的作用也和C语言的一样，就是对变量或表达式结果加一或减一
		case ZL_ST_ASM_CODE_INPRINT: //打印语句的汇编代码输出
			...
			break; //case ZL_ST_ASM_CODE_INPRINT: //打印语句的汇编代码输出
		case ZL_ST_ASM_CODE_INIF: //if-elif-else 控制语句的汇编代码输出
			if(nodes[nodenum].childs.count >= 2)
			{
				...
				{
					if(i >= ZL_AST_CHILD_NODE_SIZE) //判断是否需要查询扩展子节点，默认超过3个的子节点都存放在扩展子节点里。
					{
						tmpch = nodes[nodenum].childs.extchilds[i - ZL_AST_CHILD_NODE_SIZE];
					}
					else
						tmpch = chnum[i];
					if(nodes[tmpch].reserve == ZL_RSV_ELIF) //如果是elif就通过AsmGCElif函数来生成elif部分的汇编指令
					{
						compile->AsmGCElif(VM_ARG,&nodes[nodenum].childs,i);
						i +=2;
					}
					else if(nodes[tmpch].reserve == ZL_RSV_ELSE) //如果是else,就循环将else下面的语句生成对应的汇编指令。
					{
			...
			break; //case ZL_ST_ASM_CODE_INIF: //if-elif-else 控制语句的汇编代码输出
		case ZL_ST_ASM_CODE_INFOR: //for...endfor..循环控制语句汇编代码输出
			if(nodes[nodenum].childs.count == 4)
			{
				...
				if(chnum[0] != -1 && ZENGL_AST_ISTOKEXPRESS(chnum[0])) //例如for(i=1;i<3;i++)其中的i=1初始化部分就是第一个子节点。
					compile->AsmGenCodes(VM_ARG,chnum[0]); //生成第一个子节点对应的表达式的汇编代码。
				...
				{
					compile->AsmGenCodes(VM_ARG,i); //循环生成for...endfor之间的所有语句对应的汇编代码。
					i = nodes[i].nextnode;
				}
				...
				if(chnum[2] != -1 && ZENGL_AST_ISTOKEXPRESS(chnum[2])) //第三个子节点就是上例中的i++，循环体执行完后需要执行的代码。
			...
			break; //case ZL_ST_ASM_CODE_INFOR: //for...endfor..循环控制语句汇编代码输出
		case ZL_ST_ASM_CODE_INFUN: //fun...endfun 函数汇编代码生成
			...
			break; //case ZL_ST_ASM_CODE_INFUN: //fun...endfun 函数汇编代码生成
		case ZL_ST_ASM_CODE_INFUNCALL: //如果是函数调用，就输出相应的汇编代码。
			...
			break; //case ZL_ST_ASM_CODE_INFUNCALL: //如果是函数调用，就输出相应的汇编代码。
		case ZL_ST_ASM_CODE_INARRAY_ITEM: //生成数组元素的汇编代码。例如test[a,b+1,c] 这是一个三维的数组，会以此将a,b+1,c的值压入栈，ARRAY_ITEM寄存器指向第一个a的栈位置，由a得到第一维，由b+1的值得到第二维，由c得到第三维，所以test[a,b+1,c]类似php的test[a][b+1][c]，当然a,b+1和c都是索引而非php的数组键名。
			...
			break; //case ZL_ST_ASM_CODE_INARRAY_ITEM: //生成数组元素的汇编代码。
		...
		case ZL_ST_ASM_CODE_INRETURN: //return语句汇编输出
			...
			break; //case ZL_ST_ASM_CODE_INRETURN: //return语句汇编输出
